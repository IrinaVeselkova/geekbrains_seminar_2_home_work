...
1. __git branch__ - выводит список веток в репозитории
2. __git checkout branch.name__ - позволяет переключаться между ветками
3. __git branch branch.name__ - создает новую ветку
4. __git merge branch.name__ - позволяет объединять текущую ветку с веткой branch.name
5. __git branch -d branch.name__ - удаляет ненужную ветку
6. __git log --graph__ - визуализирует коммиты в виде графа (древа)